...
public class AdminService {
    ...
    public Set<AccountResponseDTO> getAccountPatientWithDoneSchedules() {
        List<Account> accounts = accountRepository.findPatientsWithDoneSchedules();
        ...
        for (Account account : sortedAccounts) {
            List<Schedule> doneSchedules = scheduleRepository.findDoneSchedulesByPatient(account.getId());
            if (!doneSchedules.isEmpty()) {
                AccountResponseDTO accountResponseDTO = modelMapper.map(account, AccountResponseDTO.class);
                Set<ScheduleResponse> scheduleResponseDTOS = doneSchedules.stream()
                        .map(ScheduleMapping::MapEntitytoResponse)
                        .collect(Collectors.toSet());
                accountResponseDTO.setScheduleResponses(scheduleResponseDTOS);
